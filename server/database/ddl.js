...
if (isDeleteMode) {
  ...
  // Create some users
  db.execute(
    `INSERT INTO users(first_name, last_name, mail, password) VALUES('SpungeBob', 'Square', 'mail@mail.dk', '${await bcrypt.hash(
      "123",
      12
    )}')`
  );
  db.execute(
    `INSERT INTO users(first_name, last_name, mail, password) VALUES('Patrick', 'Patrick', 'mail@mail.dk', '${await bcrypt.hash(
      "123",
      12
    )}')`
  );
